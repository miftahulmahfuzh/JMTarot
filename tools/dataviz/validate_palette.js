/*
 * ─────────────────────────────────────────────────────────────────────────────
 * VENDORED VERBATIM. Everything below this header is the `dataviz` skill's
 * `scripts/validate_palette.js`, byte for byte, and it must stay that way.
 *
 *   source:  <bundled-skills>/dataviz/scripts/validate_palette.js
 *   skill:   dataviz, bundle 2.1.220
 *   copied:  2026-07-30, by A4 (v0.5.0, roadmap A-D9)
 *   md5:     b2213861214de2489194b46f32c5a324  (of the original, without this header)
 *
 * ── WHY IT IS VENDORED AND NOT SHELLED OUT TO (A4 invariant I-22) ────────────
 *
 * A-D9 requires that the chart palette's validation run be **a committed test**:
 * *"a palette that passed once and drifted is the failure this prevents."* That
 * test has to be able to reach the validator on every `npm test`, forever.
 *
 * The skill's copy lives under `/tmp/claude-1000/bundled-skills/<version>/…` — a
 * session-scoped path with a version number in it. Shelling out to it would make
 * `chart.palette.test.ts` go RED tomorrow for a reason that has nothing to do
 * with the palette, and a test that fails for the wrong reason is a test somebody
 * deletes.
 *
 * ── AND WHY IT IS NOT RE-TYPED ───────────────────────────────────────────────
 *
 * The Machado–Oliveira–Fernandes CVD matrices below are *part of the standard*,
 * per the original's own header. A transcription slip in a 3×3 matrix does not
 * make the test fail — it makes it **pass for the wrong reason**, reporting a
 * separation that no protanope has. So: copied, not read and rewritten. Do not
 * reformat it, do not "tidy" the matrices, do not convert it to TypeScript.
 *
 * `chart.palette.test.ts` additionally reads this file's own source and asserts
 * its five thresholds and the dark lightness band, because **the cheapest way to
 * make a palette test pass is to loosen the ruler.** That is manual bookkeeping
 * with a stated reason — the `prices.ts` / `bodyHash` precedent.
 *
 * ── IT IS ESM IN A CJS PROJECT, AND THAT IS FINE HERE ────────────────────────
 *
 * `package.json` has no `"type": "module"`, so `scripts/**` is transformed to CJS
 * by tsx and CLAUDE.md records that top-level `await` is unavailable there. This
 * file is different: it is not run by tsx and it is not in `scripts/`. Node 24
 * detects ESM syntax in a `.js` file and loads it as a module, and Vitest imports
 * it directly. Measured on 2026-07-30: the CLI below runs unchanged.
 *
 * DEV-ONLY. Nothing under `src/` imports it; its one importer is a test.
 * ─────────────────────────────────────────────────────────────────────────────
 */
/**
