...
3. **Navigation uses the object form, not interpolated strings.**
   `typedRoutes` is on, and it validates against the *route* (`/[reader]`), not
   the resolved path (`/thessaly`). So this fails:

   ```ts
   router.push(`/${reader.id}/draw?service=${service.id}`)   // no
   router.push({ pathname: '/[reader]/draw',
                 params: { reader: reader.id, service: service.id } })   // yes
   ```

   The route types live in `.expo/types/router.d.ts` and are **generated by
   `expo start`, not by `expo export`**. A clean checkout has no route types, so
   `tsc` will happily pass on broken hrefs until someone starts the dev server.
   Run `npx expo start` once before trusting a typecheck that touches navigation.
...
Built: scaffold, design tokens, card dataset, deck logic, storage, `CardFan`,
`CardBack`, `Backdrop`, reader picker, service picker, draw screen with the
Daily Card gate.

Not built: onboarding, result screen, the combination rule engine, and the ~11k
words of reading copy.

The draw screen currently renders its picks inline as a placeholder. That block
is marked `TODO` and gets replaced by the result screen — it exists only so the
flow stays walkable end to end.

Reader copy note: the original spec mixed Malay into its Indonesian (`kerjaya`,
`hala tuju`, `sembang`, `awak`). `readers.json` normalizes to Indonesian
(`karier`, `arah hidup`, `ngobrol`). Keep new copy Indonesian.
