...
**MINOR rather than PATCH, and the number was argued rather than assumed.** Eleven commits
that read like bug fixes add a route, a page, a table, a cron delete and a cookie — **a new
authenticated surface**, which is a minor change whatever the commit subjects say. It was
first cut as `v0.7.1` and corrected before any release existed, because **a version number
the diff contradicts is a lie told to whoever next compares the two.** Nothing shipped breaks.

**One migration** (`0015`), taking the schema from twenty-one tables to **twenty-two**.
`LLMOp` unchanged at 13, kill switches unchanged at 7, prompt fences unchanged at 8, the
event taxonomy unchanged at 76 — a release that adds a surface and **adds nothing to any
register**. **No new dependency.** 3675 unit tests across 193 files; 659 integration across
46.
...
- **The overlay copy was corrected the same day it shipped.** It said *"Tekan Selesai di pojok
  ...
  close for them.**
- **`package-lock.json` carries the release version again.** It had been left at `0.6.0` through
  two tags, because the bump convention is `package.json` and the lockfile is not regenerated by
  a version commit. It is a cosmetic field that nothing installs against — **and a lockfile that
  disagrees with its own `package.json` is exactly the kind of small lie somebody later spends an
  afternoon on.**
